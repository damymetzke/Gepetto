...
const REGEX_VIEW_BOX = /(?:\s|,)+/gu;
const REGEX_SUBOBJECT_NOTATION = /#([1aAiI])/u;
const REGEX_SUBOBJECT_ID = /^(?:[0-9][0-9.]*[0-9]|[0-9]+)$/u;
const REGEX_VALIDATE_FINAL_NAME = /^[a-z][a-z0-9_]*$/iu;
...
export interface svgConvertInput
{
     ...
     * 
     * the name can only contain alphanumerical characters [a-z 0-9]
     * and underscores [_].
     * the first character has to be an alphabetical character [a-z].
     * 
     * when at least 1 sub-object is defined
     * the name also supports pound notation (#).
     ...
     * * #i/#I will result in roman numerals
     ...
     * so `my_object` is implicitly converted to `my_object_#1`
     * before calculateing sub-object names.
     * 
     ...
     * any g elements which are nested in other elements
     * are not part of the g-tree.
     * 
     * ```xml
     * <g id="1">
     *     <g id="3">
     ...
     * </element>
     * ```
     * in this example element 1 is part of the g-tree,
     * since its a root element.
     * element 2 is nested in a non-g element, and is not part of the g-tree.
     * element 3 is nested in a g-element which is part of the g-tree,
     * so its part of the g-tree.
     * 
     * specific g elements in the g-tree are referenced
     ...
     * 
     * sub-objects can have a comman ancestor,
     * but cannot be nested in each other.
     * so `1.2` and `1.4` can both but used as input,
     * since they only have a comman ancestor.
     * but `1.2` and `1.2.1` connot both be used as input,
     * since `1.2.1` is a decendent of `1.2`.
     * nested sub-objects will result in a rejection.
     * 
     * a notation which does not correspond to an existing sub-object
     * in the g-treewill result in a rejection.
     */
    subObjects: string[];
}
...
function numberToRoman (n: number): string {
    ...
    if (n >= upperBound - subtractOffset) {
        ...
        return numberToRoman(subtractOffset)
        + NUMERALS[index]
        + numberToRoman(n - upperBound + subtractOffset);
    ...
    return NUMERALS[index - 1].repeat(numSymbols)
    + numberToRoman(n - (previousValue * numSymbols));
 ...
 */
async function convertSingleObject (
    name: string,
...
function getSubObject (elements: Element[], index: number[]): Element[] {
    ...
    if (index.length === 0) {
        ...
        if (elements.length === 0) {
            ...
            throw new Error("Input contains empty sub-object!");
    ...
    const gElements = elements.filter((element) => element.type === "element"
        && element.name === "g");

    if (currentIndex >= gElements.length) {

        throw new Error("Input contains non-existing subobject!");
...
async function convertMultipleObjects (
    ...
    subObjects.forEach((subObject) => {
        ...
        if (!REGEX_SUBOBJECT_ID.test(subObject)) {
            ...
            throw new Error(`Subobject is improperly formatted!\nExpecting format like '1.2.3'.\nRecieved: '${subObject}'.`);
        ...
        testedObjects.forEach((testedSubObject) => {
            ...
            if (smallest === biggestSubString) {
                ...
                throw new Error("Input contains nested subobjects!");
    ...
    const subElements = subObjects.map((subObject) => {
        ...
        const index = subObject.split(".").map((value) => parseInt(value, 10));
    ...
    const results = subElements.map(async (subElement, index) => {
        ...
        const thisName = subName.replace(
            REGEX_SUBOBJECT_NOTATION,
            (_match, letter) => {
        ...
        if (!REGEX_VALIDATE_FINAL_NAME.test(thisName)) {
            ...
            throw new Error(`Invalid sub-object name: ${thisName}`);
 ...
 * it will only except filepaths
 * and only return names that can be converted to filepaths.
 * 
...
export async function convertSvg (input: svgConvertInput): Promise<string[]> {
    ...
    const topElements = xml.elements
    ...
    if (topElements.length !== 1) {
        ...
        // eslint-disable-next-line max-len
        throw new Error(`Expected a single top element called svg, got ${String(topElements.length)} instead!`);
    ...
    if (!("elements" in root)) {
        ...
        throw new Error("Svg element does not have any child elements!");
    ...
    if (viewBox.length !== 4) {
        ...
        throw new Error("viewBox attribute is improperly formatted!");
    ...
    const translateTransform1 = new TransformCommand(
    ...
    const scaleTransform = new TransformCommand(
    ...
    const translateTransform2 = new TransformCommand(
        TransformCommandType.TRANSLATE,
        {
            x: -100,
            y: -100
